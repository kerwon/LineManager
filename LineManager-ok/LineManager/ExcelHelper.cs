...
namespace LineManager
{
    public partial class ExcelHelper
    {
        ...
        public static async Task<List<KuCunInfoView>> ReadKuCunInfosExcel(string filePath)
        {
            var result = await Task.Run(() =>
            {
                List<KuCunInfoView> viewData = null;
            ...
            return result ?? throw new Exception("获取库存信息出错!");
        }
            ...
            {//出库目的地
                ...
                foreach (var xinghaoGroup in lookupXingHao)
                {
                    ...
                    foreach (var lineResultInfo in tmpList)
                    {
                        ...
                        var isWheel = lineResultInfo.LeaveLength.HasValue && lineResultInfo.LeaveLength == 0
                            ? "是"
                            : lineResultInfo.IsWheel;
